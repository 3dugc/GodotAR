...
const checks = [
	{
		...
		requirements: [
			...
			["vendor plane method list", /const\s+VENDOR_PLANE_METHODS\s*:=/],
			["vendor raycast method list", /const\s+VENDOR_RAYCAST_METHODS\s*:=/],
			["vendor anchor method list", /const\s+VENDOR_ANCHOR_METHODS\s*:=/],
			["vendor feature report helper", /func\s+_vendor_feature_report\s*\(/],
			["vendor trackable bridge report helper", /func\s+_vendor_trackable_bridge_report\s*\(/],
			["vendor plane bridge helper", /func\s+_vendor_planes\s*\(/],
			["vendor raycast bridge helper", /func\s+_vendor_raycast\s*\(/],
			["vendor anchor bridge helper", /func\s+_vendor_create_anchor\s*\(/],
			...
			["vendor trackable capability output", /capabilities\["openxr_vendor_trackable_bridge"\]\s*=\s*vendor_trackable_report/],
			["vendor raycast capability output", /capabilities\["openxr_vendor_raycast_bridge"\]\s*=\s*has_vendor_raycast/],
			["vendor anchor capability output", /capabilities\["openxr_vendor_anchor_bridge"\]\s*=\s*has_vendor_anchors/],
			...
			["vendor trackable plane feature flag", /VENDOR_TRACKABLE_PLANES/],
			["vendor raycast feature flag", /VENDOR_RAYCAST/],
			["vendor anchor feature flag", /VENDOR_ANCHORS/],
	...
	},
	{
		...
		requirements: [
			...
			["center raycast", /_center_screen_raycast/],
			["Unity-style screen raycast", /raycast_manager\.Raycast\(center,\s*raw_hits/],
			["explicit raycast camera binding", /SetRaycastCamera\(xr_camera\)/],
			["Unity async anchor placement", /TryAddAnchorAsync\(transform\)/],
			["XRI current hit metadata", /TryGetCurrent3DRaycastHit\(ray_hits\)/],
			["device profile reporting", /get_device_profile\(\)/],
			["tracking mode reporting", /get_tracking_mode\(\)/],
