...
Unity 对齐基线：以 Unity 官方可见的最新 AR Foundation / XR Core Utilities / XR Interaction Toolkit / OpenXR 文档和 alpha/beta release notes 为目标；截至 2026-06-09，官方 6000.6 alpha release notes 与当前 package docs 的观察基线为 `com.unity.xr.arfoundation@6.5.0`、`com.unity.xr.core-utils@2.6.0`、`com.unity.xr.interaction.toolkit@3.5.1`、`com.unity.xr.openxr@1.17.0`、`com.unity.xr.arcore@6.5.0`、`com.unity.xr.arkit@6.5.0`。Unity 6.5 package manuals 是当前公开 package 参考；Unity 6.4 package API pages 只在 6.5 对应 API 页面尚不可见时作为细节 fallback。如果后续出现更高的 released / pre-release / preview / unreleased 官方文档，后续周期必须前移基线，并把旧 API 作为兼容层而不是主设计。
