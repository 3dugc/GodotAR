...
- `tools/c00/install_godot_export_templates.sh` 应支持 `--download`、`--latest` 和 `--latest-stable`，把官方 Godot export templates 安装到标准模板目录，并验证 `ios.zip` 与 `android_source.zip`。C00 默认跟随最新官方 Godot 线；截至 2026-06-10 为 `4.7-rc1` / `4.7.rc1`，最新稳定 fallback 为 `4.6.3-stable` / `4.6.3.stable`。Godot editor、export templates 和 Godot source headers 必须同版本；如果最新 editor/templates 已发布但 GitHub source tag 尚未同步，iPad/ARKit 构建不得混用旧 source headers，应等待最新 source tag 或整体切到 `--latest-stable`。
