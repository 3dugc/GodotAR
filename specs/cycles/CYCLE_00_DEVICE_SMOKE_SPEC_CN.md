...
- 新增 `tools/c00/run_phase1_priority_ar_lab.sh`，作为第一优先级 iPad/ARKit + Rokid/OpenXR 现场 lane：内部复用 `run_phase1_device_lab.sh --gate all --no-audit`，设置 `INCLUDE_ANDROID_ARCORE=0`，默认等待/恢复设备并运行 `ipad` / `ipad-place` / `rokid` / `rokid-place`，输出 `C01_PRIORITY_AR_REPORT.md`；它只能证明优先真机成果，不能替代包含 Android/ARCore 的 Phase 1 completion audit。
