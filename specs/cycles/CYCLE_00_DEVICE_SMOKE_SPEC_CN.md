...
- 新增 `tools/c00/import_device_dependency_bundle.sh`，用于在 GitHub / SourceForge / Android SDK repository 下载不稳定时，从离线依赖包导入 Godot export templates、Android SDK、JDK、Godot binary 和 Godot source headers，并生成 `.godot/cache/c00/device-env.sh` 供 Rokid/iPad/Android ARCore gate 复用。
