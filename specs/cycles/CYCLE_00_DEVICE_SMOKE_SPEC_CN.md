...
- iPad readiness / device profile 必须包含 host Xcode 版本、build、`iphoneos` / `iphonesimulator` SDK 版本；当 `ddiServicesAvailable=false` 时，Next Actions 必须带上 iPadOS 与 host Xcode/SDK 组合，避免现场只得到泛化的“更新设备支持包”提示。
