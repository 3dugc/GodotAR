...
- `tools/c00/run_phase1_device_lab.sh --wait-devices` 应在 device cycle 前调用 readiness wait，等待 Rokid/iPad/Android 真机达到 ready；若超时且 `AUTO_RECOVER_DEVICES=1`，应先执行 Rokid/Android ADB recovery 和/或 iPad DDI recovery，再二次等待 readiness；仍超时才保留 readiness / recovery evidence、跳过安装/启动 cycle，并继续 completion audit，避免把离线设备误报成应用运行失败。
