...
| iOS 设备（iPad / iPhone） | Yes | Xcode project | 优先 ARKit；历史 gate 名仍为 `ipad` / `ipad-place` |
...
- Rokid/OpenXR provider 应优先消费 vendor singleton bridge 暴露的 plane/raycast/anchor 方法，并通过 `openxr_vendor_trackable_bridge`、`openxr_vendor_raycast_bridge`、`openxr_vendor_anchor_bridge` 和 `openxr_feature_flags` 记录可用入口；若这些入口没有产出命中，再进入 virtual fallback。
...
- iOS 设备（iPad 与 iPhone）导出 Xcode project，尝试 ARKit provider availability；现有 `ipad` / `ipad-place` gate 名保留为历史兼容名，但显式传 iPhone 名或 UDID 时必须能用 `--xr-platform=iphone` 运行同一套 ARKit evidence。
...
- iPhone 与 iPad 同属 C00 必需的 iOS/ARKit 设备支持范围；iPhone evidence 走同一套 ARKit gate、validator 和 device profile，不再作为可选扩展验证。
- 归档截图、录屏、日志。
...
- `tools/c00/preflight.sh`、`tools/c00/bootstrap_device_machine.sh` 和 `tools/c00/run_device_cycle.sh` 独立运行时也应自动读取 `.godot/cache/c00/device-env.sh`；可用 `C00_DEVICE_ENV_FILE` 指定路径，或用 `C00_AUTO_SOURCE_DEVICE_ENV=0` 关闭。若最新 Godot editor/templates 已发布但 iOS source tag 尚未公开，设备机允许同时存在 `.godot/cache/c00/device-env-latest.sh` 和 `.godot/cache/c00/device-env-ios-stable-fallback.sh`：Rokid/OpenXR 与 Android/ARCore gate 默认优先 latest lane，iPad/ARKit 与 iOS Simulator gate 默认优先 iOS stable fallback lane，避免为了 ARKit source 兼容把 OpenXR/Android 全部降级。
- `tools/c00/install_godot_export_templates.sh` 应支持 `--download`、`--latest` 和 `--latest-stable`，把官方 Godot export templates 安装到标准模板目录，并验证 `ios.zip` 与 `android_source.zip`。C00 默认跟随最新官方 Godot 线；截至 2026-06-11 为 `4.7-rc1` / `4.7.rc1`，最新稳定 fallback 为 `4.6.3-stable` / `4.6.3.stable`。Godot editor、export templates 和 Godot source headers 必须同版本；如果最新 editor/templates 已发布但 GitHub source tag 尚未同步，iPad/ARKit 构建不得混用旧 source headers，应等待最新 source tag 或让 iPad/ARKit 整体切到 `--latest-stable`，同时让 Rokid/OpenXR 和 Android/ARCore 继续使用 latest lane。
...
- `tools/c00/install_android_sdk_packages.sh` 应支持 `--download-cmdline-tools` / `--cmdline-tools-zip`，在设备机没有 `sdkmanager` 时安装 Android command line tools，再按选定 Godot export template 版本安装匹配的 `platform-tools`、`platforms;android-<version>`、`build-tools;<version>`，以及 template 需要的 NDK。当前 Godot `4.7.rc1` 线至少要求 `platforms;android-36`、`build-tools;36.1.0` 和 `ndk;29.0.14206865`。
- Android/Rokid preflight 默认应检查 Gradle wrapper、Android Gradle plugin 和 Kotlin Android plugin 缓存，避免 Godot headless export 在设备周期里静默下载依赖或无限等待；允许联网设备机可用 `C00_REQUIRE_ANDROID_GRADLE_CACHE=0` 放宽，但发布报告必须说明。
...
- 设备机在线依赖安装器应共用 `C00_CURL_RETRY`、`C00_CURL_RETRY_DELAY`、`C00_CURL_CONNECT_TIMEOUT`、`C00_CURL_SPEED_LIMIT`、`C00_CURL_SPEED_TIME`、`C00_CURL_MAX_TIME`、`C00_CURL_RETRY_ALL_ERRORS`、`C00_CURL_HTTP1` 和 `C00_CURL_EXTRA_ARGS`，以便在 Godot downloads / GitHub / Android SDK repository 慢速或断流时保留 partial 缓存并分段续传。
...
- Godot 导出脚本必须默认使用临时 artifact 原子导出，只有 Godot 命令成功且产物非空时才替换正式 `.apk` / `.zip`；导出失败、超时或被终止时不得拿旧 APK/Xcode zip 继续做 surface check 或真机 gate。
- `tools/c00/run_device_cycle.sh` 必须显式传播 `export_with_godot.sh` 的非零状态；任何 Android/iOS 导出失败后不得继续执行 APK/Xcode surface check、构建、安装、采集或把旧 artifact 作为本次 gate 证据。
...
- 新增 `tools/c00/run_phase1_device_lab.sh`，作为设备机第一阶段一键执行入口，按顺序编排离线依赖导入、readiness report、静态 gate、按 gate 分组的 `run_device_cycle.sh` 子进程和 completion audit，并默认纳入 `rokid-place` / `ipad-place`，支持 `--dry-run` 演练。分组子进程必须能隔离父进程中的 Godot 版本变量，确保 iPad/ARKit stable fallback 不污染 Rokid/OpenXR 与 Android/ARCore latest lane。
...
- 新增 `tools/c00/run_phase1_priority_ar_lab.sh`，作为第一优先级 iPad/ARKit + Rokid/OpenXR 现场 lane：内部复用 `run_phase1_device_lab.sh --gate all --no-audit`，设置 `INCLUDE_ANDROID_ARCORE=0`，默认等待/恢复设备并运行 `ipad` / `ipad-place` / `rokid` / `rokid-place`，输出 `C01_PRIORITY_AR_REPORT.md`；该报告必须通过 `tools/c00/append_priority_ar_diagnostics.js` 附加 readiness、recovery 和最近 iPad/Rokid smoke/placement artifacts，方便每次现场周期都可诊断、交接和发表；它只能证明优先真机成果，不能替代包含 Android/ARCore 的 Phase 1 completion audit。
- 新增 `tools/c00/import_priority_ar_evidence.sh`，用于把现场手动采集的 iPad/ARKit 与 Rokid/OpenXR 日志、截图/录屏、device profile 统一导入标准 evidence 目录，并自动运行 `verify_phase_evidence.js --gate rokid --gate ipad --gate rokid-place --gate ipad-place` 生成 `C01_PRIORITY_AR_REPORT.md`；该入口只能辅助自动 collector 失败后的证据归档，不能降低真机证据标准。
- 新增 `tools/c00/import_device_evidence.sh`，用于导入 Xcode/Console.app/Android Studio/手动录屏导出的设备证据并运行同一套 gate。
- 新增 `tools/c00/create_device_handoff_package.sh`，用于每个周期生成设备机 handoff 包：当前 APK/iPad Xcode export、runbook、spec、Unity 迁移说明、latest readiness/recovery evidence、manifest 和下一步执行命令。handoff 包只能作为阶段可运行成果，不能替代 Rokid/OpenXR、iPad/ARKit、Android/ARCore 真机证据。
- `tools/c00/run_device_cycle.sh all` 必须继续跑完 iPad/iPad placement/Rokid/Rokid placement/Android ARCore gate，并为每个子 gate 重新启动 runner、自动选择对应 device-env lane、最后执行 C00 聚合验证，避免单台失败遮住另一台设备状态，也避免 latest/stable fallback 版本混用。
- `tools/c00/run_device_cycle.sh` 应支持 `DRY_RUN=1`，用于设备机首次接入前打印 source 准备、构建、导出、采集和聚合验证编排，不调用真实设备或构建命令。
- `tools/c00/run_device_cycle.sh <ipad|ipad-place|rokid|rokid-place|android-arcore>` 在没有显式 `DEVICE` / `ADB_SERIAL` 时，应先运行 readiness probe，从 JSON 证据里自动填入唯一 iPad 或选中的 Android serial；显式参数始终优先，可用 `AUTO_SELECT_READY_DEVICE=0` 关闭该行为。
...
- iPad/ARKit gate 前应能运行 Godot source 准备链路检查；设备机没有现成 Godot source tree 时，使用 `tools/c00/prepare_godot_source.sh --tag <godot-tag>` 生成 `GODOT_SOURCE_DIR`。脚本必须先确认请求的 tag 在 Godot 官方 GitHub 仓库可用，再替换现有 source tree；`--allow-stable-fallback` 只能用于 editor/templates/source 全部切稳定版的可运行线。
- iPad/ARKit preflight 默认必须在缺少匹配 Godot source headers 时输出 `MISS`；`C00_ALLOW_PREBUILT_ARKIT=1` 只能用于显式验证已有 `GodotARKit.xcframework`，不能作为第一阶段完成证据。
...
- Android/Rokid 导出环境应在安装或复用 Godot Android build template 时清理模板默认 launcher WebP，并在 Gradle `merge*Resources` 前再次运行清理 task，避免项目 SVG 图标导出的 PNG 与 `mipmap*/icon.webp` / `icon_foreground.webp` 产生 duplicate resources。
...
- `tools/c00/run_phase1_priority_ar_lab.sh`
