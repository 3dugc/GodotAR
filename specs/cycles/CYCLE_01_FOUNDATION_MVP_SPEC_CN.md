...
Unity 对齐基线：以 Unity 官方可见的最新 AR Foundation / XR Core Utilities / XR Interaction Toolkit / OpenXR 文档和 alpha/beta release notes 为目标；当前 C01 以官方 6000.6 alpha release notes 中的 Unity 6.x `com.unity.xr.arfoundation@6.5.0`、`com.unity.xr.interaction.toolkit@3.5.0`、`com.unity.xr.openxr@1.17.0` 观察基线为目标，并用 Unity 6.4 package manuals 补足公开 API 细节。`XROrigin` 是主入口，`ARSessionOrigin` 只作为 deprecated 迁移 shim。若官方出现更高 released、pre-release、preview 或 unreleased 文档，后续 spec 必须前移基线。
