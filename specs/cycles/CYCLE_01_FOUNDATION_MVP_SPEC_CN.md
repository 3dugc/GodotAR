...
- `tools/c00/run_phase1_priority_ar_lab.sh` 已落地 iPad/ARKit + Rokid/OpenXR 第一优先级真机 lane；它默认等待并恢复设备、运行 `ipad` / `ipad-place` / `rokid` / `rokid-place`，输出 `C01_PRIORITY_AR_REPORT.md`，并通过 `append_priority_ar_diagnostics.js` 追加 readiness/recovery/smoke artifact 诊断附录，但不替代包含 Android/ARCore 的完整 Phase 1 completion audit。
...
| C01 优先真机 lane | 设备机 | `tools/c00/run_phase1_priority_ar_lab.sh --device "iPad M4" --wait-devices` | 生成带 `Priority Lane Diagnostics` 附录的 `C01_PRIORITY_AR_REPORT.md`，并验证 `ipad`、`ipad-place`、`rokid`、`rokid-place`；Android/ARCore 仍由完整 Phase 1 audit 补齐 |
