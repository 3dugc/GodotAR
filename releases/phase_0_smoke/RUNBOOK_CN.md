...
如果 iPad/ARKit 因最新 Godot source tag 尚未公开而必须走稳定 fallback，不要让 Rokid/OpenXR 和 Android/ARCore 跟着降级。可在设备机保留 `.godot/cache/c00/device-env-latest.sh` 和 `.godot/cache/c00/device-env-ios-stable-fallback.sh` 两个本地 lane 文件；`preflight.sh` / `run_device_cycle.sh` 会让 `rokid`、`rokid-place`、`android-arcore` 优先读取 latest lane，让 `ipad`、`ipad-place` 和 iOS Simulator gate 优先读取 iOS stable fallback lane。显式 `C00_DEVICE_ENV_FILE=...` 仍然优先于自动选择。
...
例如 Godot 版本为 `4.7.rc1.official` 时，tag 通常是 `4.7-rc1`；稳定版 `4.6.3.stable.official` 对应 `4.6.3-stable`。脚本会先确认远端 source tag 可用，再替换默认 source 目录；截至 2026-06-11，Godot 官方 GitHub 仓库仍未公开 `4.7-rc1` source tag，所以 iPad/ARKit fallback 必须让 editor、templates、source headers 全部使用 `--latest-stable`。Rokid/OpenXR 和 Android/ARCore 可以继续使用 `device-env-latest.sh` 的 4.7 lane。脚本会输出 `GODOT_SOURCE_DIR=... ios/plugins/godot_arkit/build_xcframework.sh`，后续 iPad gate 使用同一个 `GODOT_SOURCE_DIR`。
...
Rokid/Android ready 条件是 ADB 出现 `device` 状态的已授权设备；单个 `rokid` / `android-arcore` readiness gate 会把选中的 `selected_device.serial` 继续作为 `ADB_SERIAL` 传给安装、启动、日志和媒体采集。iPad ready 条件是 devicectl/xctrace 不再显示 `offline` / `unavailable`。`--run-gate` 会在 ready 后调用 `tools/c00/run_device_cycle.sh`，并继续按本 runbook 的证据规则归档。
...
Rokid/Android readiness 会记录 ADB 版本、Android SDK 环境、JAVA_HOME、PATH 中是否有 `adb`，并在 macOS 上尝试列出 USB 中疑似 Android/XR 的设备；如果 USB 能看到设备但 ADB 没有 transport，优先处理 USB debugging、RSA 授权和 USB 模式，而不是先重装 APK。多台 Android/XR 设备同时连接时，仍建议显式设置 `ADB_SERIAL=<serial>`，确保 Rokid/OpenXR 和 Android/ARCore 分别跑到预期设备；`run_phase1_device_lab.sh --gate all --wait-devices` 的 split-all 模式会按 Rokid 组和 Android ARCore 组分别传播 readiness 选中的 serial，运行完每组后恢复原始 `ADB_SERIAL`。
也可以运行 `node tools/c00/recover_android_adb_transport.js --gate rokid` 或 `--gate android-arcore` 进行 Rokid/Android ADB recovery；该工具会保存恢复前后 readiness，执行 `adb kill-server` / `adb start-server` / `adb devices -l`，并把 stdout/stderr 和下一步动作写进 summary。若追加 `--run-gate`，它只会在恢复后 readiness 通过时继续执行对应 device gate。
如果目标 iPad 已在 `devicectl list devices` 里出现但状态是 `unavailable`，后续 `CoreDeviceService was unable to locate a device matching the requested device identifier` 只是不可用设备的连锁症状；只有明确的 `Operation not permitted`、`XPCError`、Instruments cache 或 `permission denied` 才按 `host_permission_blocked` 处理。
...
如果本次现场只要求先把最高优先级的 iPad/ARKit 与 Rokid/OpenXR 跑通，使用 priority AR lane：
...
`all` 会按 iPad/ARKit、iPad C04 placement、Rokid/OpenXR、Rokid C02 placement、Android/ARCore 顺序执行；每个子 gate 会重新启动 runner、清掉父进程中的 Godot 版本变量，并读取对应 device-env lane。如需临时跳过 Android ARCore，设置 `INCLUDE_ANDROID_ARCORE=0`。如需临时跳过 placement 专项 demo gate，设置 `INCLUDE_PLACE_DEMOS=0`，但这只适合调试，不能作为第一阶段完整通过。
...
`IOS_SIMULATOR_ARCHS=auto` 会让构建脚本检测 Godot simulator template 的 slice，并优先选择当前主机能安装到 simulator 的架构；Apple Silicon 默认选择 `arm64`。特殊诊断时可显式设置 `IOS_SIMULATOR_ARCHS=x86_64` 或其它架构列表。
