...
readiness 和 device profile 报告里的 `Next Actions` 是现场恢复清单：例如接入/授权 ADB、解锁并信任 iPad、打开 Xcode Devices and Simulators、处理 `ddiServicesAvailable=false` 或等待 gate 安装目标 app。iPad readiness 会同时记录 host Xcode 版本、build、`iphoneos` / `iphonesimulator` SDK 版本；如果 `ddiServicesAvailable=false`，优先用报告里的 iPadOS 与 Xcode/SDK 组合判断是否需要安装匹配设备支持包、更新 Xcode/Xcode beta，或更新 iPadOS。
