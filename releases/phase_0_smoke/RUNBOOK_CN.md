...
也可以运行 `node tools/c00/recover_ios_ddi_services.js --device "iPad M4"` 进行 iPad DDI recovery；该工具会保存 auto-mount 前后 readiness、`devicectl` JSON/log 和 summary。若追加 `--run-gate`，它只会在恢复后 iPad readiness 通过时继续执行 iPad gate。
