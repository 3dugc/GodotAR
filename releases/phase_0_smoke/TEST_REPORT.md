...
- The local cached device-lab editor is still Godot 4.4.1 stable and runs successfully outside the Codex sandbox: `4.4.1.stable.official.49a5bc7b6`. This is retained as legacy evidence; new device-machine setup should install the C00 default latest line, currently `4.7-rc1`, unless the operator explicitly pins a fallback version.
- Local Godot source headers for `4.4.1-stable` are prepared under `.godot/cache/c00/godot-source`; iPad/ARKit builds against `4.7-rc1` must refresh this tree with `tools/c00/prepare_godot_source.sh --latest` or `GODOT_TAG=4.7-rc1`.
...
- Offline device-machine setup is still supported through `tools/c00/import_device_dependency_bundle.sh --bundle <device-bundle-dir>`. Put a matching Godot export templates package such as `Godot_v4.7-rc1_export_templates.tpz`, Android SDK `platform-tools`/`build-tools`, a real JDK, optional matching `Godot.app`, and optional matching `godot-source` into the bundle, then either source `.godot/cache/c00/device-env.sh` manually or let the C00 entry scripts auto-source it.
