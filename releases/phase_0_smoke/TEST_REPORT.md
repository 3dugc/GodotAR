...
- No matching local Godot source tree is currently accepted for iPad/ARKit 4.7.rc1. `tools/c00/prepare_godot_source.sh --tag 4.7-rc1 --force` was attempted on 2026-06-10, but the public Godot GitHub source tag was not available; `preflight.sh ipad` therefore reports `MISS Godot source headers for 4.7.rc1`.
...
- Existing Rokid/OpenXR APK `builds/rokid/c00.apk` still passes APK static inspection for `--xr-platform=rokid`, OpenXR loader/vendor libraries, and no ARCore native libraries, but it is a legacy artifact from before the 4.7.rc1 Android build-template refresh and is not counted as a fresh 4.7.rc1 export.
- Legacy Android/ARCore export evidence produced `builds/android_arcore/c00.apk`; APK static inspection confirmed `--xr-platform=arcore`, ARCore native libraries, and no OpenXR loader. Fresh 4.7.rc1 Android/ARCore export still needs to be re-run after the headless export blocker is fixed.
...
- Godot source headers are now version-checked against the selected export template version in `preflight.sh`, `bootstrap_device_machine.sh`, `run_device_cycle.sh`, `import_device_dependency_bundle.sh`, and `ios/plugins/godot_arkit/build_xcframework.sh`; on 2026-06-10, the official Godot archive lists `4.7-rc1`, but the public Godot GitHub source tag was not available through `git ls-remote`, so iPad/ARKit must either wait for the `4.7-rc1` source tag or use the fully matching `4.6.3-stable` fallback line. Mixing `4.6.3` source headers with `4.7.rc1` export templates is treated as NOT_READY.
- Fresh 4.7.rc1 Rokid/OpenXR headless Android export currently blocks inside Godot after printing the OpenXR hand-tracking setting warning and Android build-tools fallback messages; the headless Godot process stays near full CPU and does not update `builds/rokid/c00.apk`. This is recorded as an export-toolchain blocker, not as a device runtime failure.
...
## Local Verification Through 2026-06-10
...
| `node tools/c00/run_static_gates.js --gate all --format json` | Pass | Re-run on 2026-06-10; includes `git diff --check`, Unity latest reference baseline, ARFoundation/XRI surface checks, export preset checks, ARKit artifact checks, Android export surface checks, 4.7 source-version guards, and atomic export surface |
...
| `tools/c00/preflight.sh rokid` | Pass | Re-run on 2026-06-10 with Godot `4.7.rc1`, 4.7 templates, Android SDK/JDK, debug keystore, OpenXR Vendors addon, Khronos AAR, Android build template `4.7.rc1`, and Rokid preset present |
| `tools/c00/preflight.sh ipad` | Fail as expected / NOT_READY | Re-run on 2026-06-10: iOS template and ARKit plugin config are present, but matching `4.7.rc1` Godot source headers are missing because the `4.7-rc1` public source tag was not available; iPad also still needs a real Apple Team ID before device install |
| `RUN_COLLECT=0 RUN_PHASE_VERIFY=0 tools/c00/run_device_cycle.sh rokid` | Blocked / diagnostic produced | On 2026-06-10 fresh 4.7.rc1 headless Android export stayed near full CPU for more than two minutes and did not update `builds/rokid/c00.apk`; the process was terminated and the old APK is not counted as a fresh export |
...
| `tools/c00/export_with_godot.sh "C02 Rokid OpenXR Place" builds/rokid/c02-place.apk` | Legacy pass | 2026-06-09 placement APK export completed with `--xr-scene=rokid_place` and OpenXR mode; fresh 4.7.rc1 placement export should wait until the headless Android export blocker is fixed |
...
| `tools/c00/export_with_godot.sh "C00 Android ARCore" builds/android_arcore/c00.apk` | Legacy pass | 2026-06-09 export completed with GodotARCore enabled only for ARCore preset; fresh 4.7.rc1 ARCore export should wait until the headless Android export blocker is fixed |
